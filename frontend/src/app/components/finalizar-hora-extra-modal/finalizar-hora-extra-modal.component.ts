...
@Component({
  selector: 'app-finalizar-hora-extra-modal',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './finalizar-hora-extra-modal.component.html',
  styleUrls: ['./finalizar-hora-extra-modal.component.scss']
})
export class FinalizarHoraExtraModalComponent implements OnInit {
  form!: FormGroup;
  cargando = false;
  capturandoGPS = false;
  ...
  videoinput: any = null;
  horaExtraId: number = 0;

  constructor(
    private fb: FormBuilder,
    private horaExtraService: HoraExtraService,
    private snackBar: MatSnackBar,
    public dialogRef: MatDialogRef<FinalizarHoraExtraModalComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any
  ) {
    this.horaExtraId = data?.horaExtraId || 0;
    this.initForm();
  }

  ngOnInit(): void {
    if (!this.horaExtraId) {
      this.snackBar.open('Error: No se especificó la hora extra a finalizar', 'Error', { duration: 5000 });
  ...
  /**
   * Guardar finalización de hora extra
   */
  guardar(): void {
    if (!this.form.valid) {
    ...
    if (!this.fotoCapturada) {
      this.snackBar.open('Debes capturar una foto final', 'Error', { duration: 5000 });
      return;
    }
    ...
    const fotoFile = new File([blob], 'hora-extra-fin.jpg', { type: 'image/jpeg' });

    this.horaExtraService.finalizarHoraExtra(
      this.horaExtraId,
      ...
      error: (error) => {
        this.cargando = false;
        const message = error.error?.message || 'Error al finalizar hora extra';
